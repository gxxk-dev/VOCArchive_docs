# VOCArchive(Future ver.) TODO

我(gxxk-dev)将在此处记录一些需要进一步商讨/计划的功能。
如果你有一些好的点子与建议，欢迎在[Github Repo/VOCArchive_Future Discussion](https://github.com/gxxk-dev/VOCArchive_Future/discussions)中提出。

1. 数据质量优化
    1. 多源异构数据的实体对齐
        1. 聚合体 机制：返回同一实体下关联的多个对象
    2. workRelation防灌水
    3. 信任网络
2. 更加智能的搜索
    1. 向量搜索
    2. 音频指纹
3. 发布者身份验证
    1. 去中心化的发布者身份验证(DID:Decentralized Identifiers)
    2. 由用户控制的权限验证网络(UCAN:User Controlled Authorization Networks)
7. Repo Mode: 将大量数据实体聚合在一个统一的身份下进行发布和更新
8. Atom Mode: 每个独立实体拥有独立主权